...
function PortfolioCard({ item }: { item: PortfolioItem }) {
  ...
  const category = locale === "id" ? item.category : item.categoryEn;
  const description = locale === "id" ? item.description : item.descriptionEn;

  return (
    <article
      key={item.id}
      data-testid="portfolio-card"
      className="bg-neutral-50 rounded-lg shadow-soft overflow-hidden flex flex-col"
    >
      <div className="relative h-48 bg-gradient-to-br from-primary-100 to-primary-200 overflow-hidden">
        {item.image && !imgError ? (
          ...
          <div className="flex h-full items-center justify-center text-primary-700 font-medium text-sm">
      ...
      <div className="p-6 flex flex-col flex-1">
        ...
        <a
          ...
          className="mt-4 inline-flex items-center gap-2 text-sm font-semibold text-primary-700 transition-colors rounded focus:outline-none focus-visible:ring-2 focus-visible:ring-accent-400 focus-visible:ring-offset-2 focus-visible:ring-offset-neutral-50"
        >
